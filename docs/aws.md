...
| `transitions.json` | One bundle per departure, when the review read a source beyond Okta. Hashed like the rest, so it can be shown to be the one the review produced. **Not produced by the AWS pipeline yet:** `handlers.collect` passes no GitHub snapshot. The graph checks themselves do run there — the graph is built from Okta alone — so AR-18 reports an Okta API client whose owner left, and a register entry naming GitHub is recorded as a gap. The bundle needs a second estate to mean anything, so it appears only in CLI runs given `--github` |
